...
//	DONE_OR_NOT();
	...
	while(1){
		...
		if(id!=1 && Rece_Proposal_flag) break;
...
//		if(Commit_Truth_Set_Number >= 2*f+1 && !COMMIT_flag)break;
	...
	while(1){
		...
		if(id==1 && NEXT_ROUND_flag && Viewchange_Number == Need_to_Wait){
			if(Total_time==0)
				Total_time = TIM2->CNT + TIM2_Exceed_Times * 9000;
			NEXT_ROUND_flag = 0;
			Leader_Send(NEXT_ROUND,Viewchange_id,Viewchange_Sig);
			
		}else if(id!=1 && NEXT_ROUND_flag){
			Send_VIEW_CHANGE();
			NEXT_ROUND_flag=0;
		}else{
			NACK_phase();
//			if(id==1)Send_REPLAY(Total_time1,Total_time,TIM2_Exceed_Times);
		}
...
void NACK_phase(){
	...
	if(id == 1 && Commit_Truth_Set_Number >= 2*f+1){
