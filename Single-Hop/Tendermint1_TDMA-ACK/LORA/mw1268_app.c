...
//						ACK_TO_Send_Number = ACK_TO_Send_Number + 1;
					...
					ACK_My_Choice = uart3RxBuffer[id + 3];
					ACK_My_Commit = uart3RxBuffer[id + 3 + N];
					
					if(id == 1){
						if((ACK_My_Proposal == 1) && (ACK_Proposal[temp_id] == 0xFF)){
							ACK_Proposal[temp_id] = 0x01;
							ACK_Proposal_Number = ACK_Proposal_Number + 1;
						}
					}
					if( (ACK_My_Choice == 1) && (ACK_Choice[temp_id] == 0xFF)){
						ACK_Choice[temp_id] = 0x01;
						ACK_Choice_Number = ACK_Choice_Number + 1;
					}
					if((ACK_My_Commit == 1) && (ACK_Commit[temp_id] == 0xFF)){
						ACK_Commit[temp_id] = 0x01;
