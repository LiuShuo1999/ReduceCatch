...
//					}
					...
					u8 old_value;
					if((id == 1) && (Prepare_Number <= N-1) && (Rece_Prepare[temp_id]==0xFF) && (temp_value==1)){
...
//							hash[3] = 1;
					...
					u8 temp_value = uart3RxBuffer[3];
					if((id == 1) && (Viewchange_Number <= N-1) && (Rece_Viewchange[temp_id]==0xFF) && (temp_value==1)){
...
//				else if(verify_flag && (temp_type == DECIDE)){
//					u8 temp_value = uart3RxBuffer[3];
//					if( (id != 1) && (temp_id == 1)){
//						//验证2f个签名
//						for(u8 m=0;m<temp_datasize;m++)PreCommit_id[m] = uart3RxBuffer[3+m];
//						for(u8 x=0;x<temp_datasize;x++){
//							for(u8 y=0;y<Sig_Size;y++)
//								PreCommit_Sig[x][y] = uart3RxBuffer[3+temp_datasize+Sig_Size+x*Sig_Size+y];
//						}
//						u8 verify_sig = 1;
//						for(u8 m=0;m<temp_datasize;m++){
//							memset(hash,0,32);
...
//							hash[2] = 1;
//							hash[3] = 1;
//							if (!uECC_verify(public_key[PreCommit_id[m]], hash, 4, PreCommit_Sig[m], uECC_secp192r1())) {
//								verify_sig = 0;
//								break;
//							}
//						}
...
//								}
//							}
//							if(Precommit_Number>=2*f+1)Rece_decide = 1;
//						
//						} 
////						if(verify_sig) Rece_decide = 1;
//						
