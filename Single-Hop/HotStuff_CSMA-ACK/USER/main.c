...
//任务优先级
#define CSMACA_HOTSTUFF_TASK_PRIO		2
//任务堆栈大小	
#define CSMACA_HOTSTUFF_STK_SIZE 		512  
//任务句柄
TaskHandle_t CSMACA_HOTSTUFF_Task_Handler;
//任务函数
void CSMACA_HOTSTUFF(void *pvParameters);
...
void start_task(void *pvParameters)
{
    ...
    //创建LED0任务
    xTaskCreate((TaskFunction_t )CSMACA_HOTSTUFF,     	
                (const char*    )"CSMACA_HOTSTUFF",   	
                (uint16_t       )CSMACA_HOTSTUFF_STK_SIZE, 
                (void*          )NULL,				
                (UBaseType_t    )CSMACA_HOTSTUFF_TASK_PRIO,	
                (TaskHandle_t*  )&CSMACA_HOTSTUFF_Task_Handler);   
//    //创建LED1任务
