...
//任务优先级
#define CSMA_HOTSTUFF_TASK_PRIO		2
//任务堆栈大小	
#define CSMA_HOTSTUFF_STK_SIZE 		512  
//任务句柄
TaskHandle_t CSMA_HOTSTUFF_Task_Handler;
//任务函数
void CSMA_HOTSTUFF(void *pvParameters);
...
void start_task(void *pvParameters)
{
    ...
    //创建LED0任务
    xTaskCreate((TaskFunction_t )CSMA_HOTSTUFF,     	
                (const char*    )"CSMA_HOTSTUFF",   	
                (uint16_t       )CSMA_HOTSTUFF_STK_SIZE, 
                (void*          )NULL,				
                (UBaseType_t    )CSMA_HOTSTUFF_TASK_PRIO,	
                (TaskHandle_t*  )&CSMA_HOTSTUFF_Task_Handler);   
