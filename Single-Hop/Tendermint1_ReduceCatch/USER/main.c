...
//任务优先级
#define TENDERMINT1_TASK_PRIO		2
//任务堆栈大小	
#define TENDERMINT1_STK_SIZE 		512  
//任务句柄
TaskHandle_t TENDERMINT1_Task_Handler;
//任务函数
void TENDERMINT1(void *pvParameters);
...
uint8_t private_key[20][32] = {0};
uint8_t public_key[20][64] = {0};
...
void start_task(void *pvParameters)
{
    ...
    //创建LED0任务
    xTaskCreate((TaskFunction_t )TENDERMINT1,     	
                (const char*    )"TENDERMINT1",   	
                (uint16_t       )TENDERMINT1_STK_SIZE, 
                (void*          )NULL,				
                (UBaseType_t    )TENDERMINT1_TASK_PRIO,	
                (TaskHandle_t*  )&TENDERMINT1_Task_Handler);   
//    //创建LED1任务
