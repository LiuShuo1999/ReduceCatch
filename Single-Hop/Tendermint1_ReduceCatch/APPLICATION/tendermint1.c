...
void Send_VC(){
	...
	for(u8 m=0;m<64;m++){
		Send_Data_buff[m+Data_Size+3] = sig[m];
	}
...
//	}
	...
	for(u8 ntx=0;ntx<NTX;ntx++){
		...
		if(id==1 && Viewchange_Number == Need_to_Wait  && Rece_Proposal_flag){
			if(Total_time == 0 ){
				Total_time = TIM2->CNT + TIM2_Exceed_Times * 9000;
			}
	...
	if(id==1 && Total_time == 0 && Viewchange_Number == Need_to_Wait){
		Total_time = TIM2->CNT + TIM2_Exceed_Times * 9000;
	}
...
//		AfterRound();
	...
	if(id!=1 && Rece_next==1 && Total_time==0  && Rece_Proposal_flag){
		Total_time = TIM2->CNT + TIM2_Exceed_Times * 9000;
	}
...
//}
void test_finish(){
	if(Commit_Truth_Set_Number >= Need_to_Wait && Total_time1==0){
		Total_time1 = TIM2->CNT + TIM2_Exceed_Times * 9000;
//		decided = 1;
	}
}
