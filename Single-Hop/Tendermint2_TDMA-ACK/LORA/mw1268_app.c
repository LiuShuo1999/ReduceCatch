...
//					}
					...
					u8 old_value;
					if((id == 1) && (Prepare_Number <= N) && (Rece_Prepare[temp_id]==0xFF) && (temp_value==1)){
						...
						//回复ACK，
						if(ACK_TO_Send_Number >= 5)ACK_TO_Send_Number=5;
...
//							hash[3] = 1;
					...
					u8 temp_value = uart3RxBuffer[3];
					if((id == 1) && (Viewchange_Number <= N) && (Rece_Viewchange[temp_id]==0xFF) && (temp_value==1)){
