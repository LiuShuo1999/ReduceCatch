...
///////////////////////////////////////////////////////////////////////////////////////////////////
			if(LoRa_CFG.tmode == LORA_TMODE_PT){
				...
				if(uart3RxBuffer[1]==NACK){
					u8 x = 0;
					x = x+1;
				}
				
				// 处理数据
				if(verify_flag && !verify_flag2 && uart3RxBuffer[1]==NACK && uart3RxBuffer[3]==PREPREPARE){
					Need_proposal = 1;
				}
...
//					}
					...
					u8 old_value;
					if((id == Group_Leader_ID) && (Prepare_Number <= 2*f) && (Rece_Prepare[temp_id]==0xFF) && (temp_value==1)){
...
//							hash[3] = 1;
					...
					u8 temp_value = uart3RxBuffer[3];
					if((id == Group_Leader_ID) && (Commit_Number <= 2*f) && (Rece_Commit[temp_id]==0xFF) && (temp_value==1)){
...
//									Total_time = TIM2->CNT + TIM2_Exceed_Times * 9000;
					...
					u8 temp_value = uart3RxBuffer[3];
					if((id == Group_Leader_ID) && (Viewchange_Number <= 2*f) && (Rece_Viewchange[temp_id]==0xFF) && (temp_value==1)){
...
//						ACK_TO_Send_Number = ACK_TO_Send_Number + 1;
				...
				else if(verify_flag2 && verify_flag && (temp_type == NACK)){
					if(id==Group_Leader_ID && temp_id != Group_Leader_ID){
					...
					}else if(id != Group_Leader_ID && temp_id == Group_Leader_ID){
						u8 catching_type = uart3RxBuffer[3];
//						u8 temp_id_idx = temp_id - 4 * (Group_Number - 1);
						u8 my_id = 3 + id - 4 * (Group_Number - 1);
...
//					}
//				}
				else if(verify_flag && !verify_flag2 && uart3RxBuffer[1]==GLOBAL_NACK && uart3RxBuffer[3]==GLOBAL_PREPREPARE){
					Need_proposal_2 = 1;
				}
					...
					if(Global_Rece_Proposal_Number[tmp_group_number] == Global_Proposal_Packet_Number[tmp_group_number]){
						...
						if(Global_Rece_Sig_Number[tmp_group_number]>=2*f){
...
//					Global_ACK_TO_Send_Number = Global_ACK_TO_Send_Number + 1;
				...
				else if(verify_flag2 && verify_flag && (temp_type == GLOBAL_PREPARE)){
					//Prepare	Choice
					u8 temp_value = uart3RxBuffer[3];
					u8 old_value;
...
//						Global_ACK_TO_Send[Global_ACK_TO_Send_Number][0] = id;
//						Global_ACK_TO_Send[Global_ACK_TO_Send_Number][1] = GLOBAL_ACK;
//						Global_ACK_TO_Send[Global_ACK_TO_Send_Number][2] = GLOBAL_PREPARE;
//						Global_ACK_TO_Send[Global_ACK_TO_Send_Number][3] = temp_id;
//						Global_ACK_TO_Send[Global_ACK_TO_Send_Number][4] = 0x00;
//						Global_ACK_TO_Send_Number = Global_ACK_TO_Send_Number + 1;
					}
...
//						Global_ACK_TO_Send_Number = Global_ACK_TO_Send_Number + 1;
					...
					if( (id != Global_Consensus_Leader) && (temp_id == Global_Consensus_Leader)){
						//验证2f个签名
						u8 Total_ID = uart3RxBuffer[3];
						for(u8 m=0;m<Total_ID;m++)Global_Commit_id[m] = uart3RxBuffer[4+m];
						for(u8 x=0;x<Total_ID;x++){
							for(u8 y=0;y<Sig_Size;y++)
								Global_Commit_Sig[x][y] = uart3RxBuffer[3+1+Total_ID+x*Sig_Size+y];
						}
						u8 verify_sig = 1;
						for(u8 m=0;m<Total_ID;m++){
							u8 temp_buff[100];
							temp_buff[0] = Global_Commit_id[m];
							temp_buff[1] = GLOBAL_COMMIT;
							temp_buff[2] = 1+32;
							temp_buff[3] = 1;
							for(u8 k=0;k<32;k++)temp_buff[4+k]=Proposal_2_hash[k];
							sha2(temp_buff,36,hash,0);
							if (!uECC_verify(public_key[Global_Commit_id[m]], hash, 32, Global_Commit_Sig[m], uECC_secp192r1())) {
								verify_sig = 0;
								break;
							}
						}
						if(verify_sig){
							...
							for(u8 m=0;m<Total_ID;m++){
								if(Global_Rece_Commit[Global_Commit_id[m]]==0xFF){
									Global_Rece_Commit[Global_Commit_id[m]]=1;
									Global_Commit_Number = Global_Commit_Number+1;
								}
							}
							if(Global_Commit_Number>=2*Global_f+1){
...
//						if(verify_sig) Rece_decide = 1;
					...
					if( (id != Global_Consensus_Leader) && (temp_id == Global_Consensus_Leader)){
						...
						if(verify_sig){
							...
							if(Global_Viewchange_Number>=2*Global_f+1){
								Global_Rece_next = 1;
								if(Total_time==0)
									Total_time = TIM2->CNT + TIM2_Exceed_Times * 9000;
							}
					...
					u8 temp_value = uart3RxBuffer[3];
					if((id == Global_Consensus_Leader) && (Global_Viewchange_Number <= 2*Global_f) && (Global_Rece_Viewchange[temp_id]==0xFF) && (temp_value==1)){
...
//						Global_ACK_TO_Send_Number = Global_ACK_TO_Send_Number + 1;
				...
				else if(verify_flag2 && verify_flag && (temp_type == GLOBAL_NACK)){
					if(id==Global_Consensus_Leader && temp_id != Global_Consensus_Leader){
						u8 catching_type = uart3RxBuffer[3];
						if(catching_type == GLOBAL_PREPREPARE){
							Need_proposal_2 = 1;
						}else if(catching_type == GLOBAL_PRECOMMIT){
							Global_Need_precommit = 1;
						}else if(catching_type == GLOBAL_COMMIT){
							Global_Need_commit = 1;
						}else if(catching_type == GLOBAL_DECIDE){
							Global_Need_decide = 1;
						}else if(catching_type == GLOBAL_NEXT_ROUND){
							Global_Need_next = 1;
						}
					}else if(id != Global_Consensus_Leader && temp_id == Global_Consensus_Leader){
						u8 catching_type = uart3RxBuffer[3];
						u8 my_id = 3 + Group_Number;
						if(catching_type == GLOBAL_PREPARE && uart3RxBuffer[my_id]==0xFF){
							Global_Need_PREPARE = 1;
						}else if(catching_type == GLOBAL_PRECOMMIT && uart3RxBuffer[my_id]==0xFF){
							Global_Need_PRECOMMIT = 1;
						}else if(catching_type == GLOBAL_COMMIT && uart3RxBuffer[my_id]==0xFF){
							Global_Need_COMMIT = 1;
						}else if(catching_type == GLOBAL_DECIDE && uart3RxBuffer[my_id]==0xFF){
							Global_Need_VIEWCHANGE = 1;
						}
					}
				}	
