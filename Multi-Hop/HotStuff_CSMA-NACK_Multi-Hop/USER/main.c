...
//任务优先级
#define HOTSTUFF_TASK_PRIO		2
//任务堆栈大小	
#define HOTSTUFF_STK_SIZE 		512  
//任务句柄
TaskHandle_t HOTSTUFF_Task_Handler;
//任务函数
void HOTSTUFF(void *pvParameters);
...
void start_task(void *pvParameters)
{
    ...
    //创建LED0任务
    xTaskCreate((TaskFunction_t )HOTSTUFF,     	
                (const char*    )"HOTSTUFF",   	
                (uint16_t       )HOTSTUFF_STK_SIZE, 
                (void*          )NULL,				
                (UBaseType_t    )HOTSTUFF_TASK_PRIO,	
                (TaskHandle_t*  )&HOTSTUFF_Task_Handler);   
//    //创建LED1任务
