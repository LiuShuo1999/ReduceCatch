...
/*****************************Local_Consensus**************************************************************************************************************************************************************************/
	...
	while(1){
		...
		if(id==Group_Leader_ID && tmp_flag!=2)
			Propose(0);
		else{
			delay_ms(300);
			if(Rece_Proposal_flag==1)break;
...
/**************************PRECOMMIT**************************/
	...
	while(1){
		...
		if((id != Group_Leader_ID) && ((ACK_precommit == 1) || (Rece_decide == 1) || (Rece_next == 1)))break;
		...
		if(id==Group_Leader_ID && precommit_flag){
			u8 tmp_flag = 0;
...
/**************************Decide**************************/
	...
	while(1){
		...
		if(id == Group_Leader_ID && Viewchange_Number >= Need_to_Wait) break;
		
		BeforeRound();

		if(id != Group_Leader_ID && !(Rece_decide == 1)){
			AfterRound();
			continue;
		}
		
		if(id==Group_Leader_ID && DECIDE_flag){
			u8 tmp_flag = 0;
			while(ACK_TO_Send_Number){
				Go_ACK();
				delay_ms(50);
...
/*****************************Global_Consensus**************************************************************************************************************************************************************************/
	...
	/**************************PREPREPARE**************************/
	Tendermint_status = GLOBAL_PREPREPARE;
	...
	tmp_flag = 1;
	while(1){
		if(Global_Prepare_Number >= GROUPS_NUMBER){Rece_Proposal_2_flag = 1;break;}
		...
		if(id == Global_Consensus_Leader && tmp_flag!=2)
			Global_Propose();
		else{
			delay_ms(300);
			if(Rece_Proposal_2_flag==1)break;
		}
...
//	run_slow();
	...
	/**************************PREVOTE**************************/
	Tendermint_status = GLOBAL_PREVOTE;
	my_choice = 1;
	...
	while(1){
		...
		if((Global_ACK_prepare == 1) || (Global_Rece_precommit == 1) || (Global_Rece_decide == 1) || (Global_Rece_next == 1)) break;
		
		BeforeRound();
		
		if((Global_ACK_prepare == 1) || (Global_Rece_precommit == 1) || (Global_Rece_decide == 1) || (Global_Rece_next == 1)) break;
		
		Global_Follower_Send(GLOBAL_PREVOTE,1);
...
/**************************PRECOMMIT**************************/
	Tendermint_status = GLOBAL_PRECOMMIT;
	my_precommit = 1;
	precommit_flag = 0;
	while(1){
		
		if((id != Global_Consensus_Leader) && ((Global_ACK_precommit == 1) || (Global_Rece_decide == 1) || (Global_Rece_next == 1)))break;
		...
		if(id == Global_Consensus_Leader && precommit_flag){
			u8 tmp_flag = 0;
...
/**************************Decide**************************/
	Tendermint_status = GLOBAL_DECIDE;
	my_decide = 1;
	decide_flag = 1;
	DECIDE_flag = 0;
	while(1){
		...
		if(id == Global_Consensus_Leader && Global_Viewchange_Number >= GROUPS_NUMBER) break;
		
		BeforeRound();

		if(id != Global_Consensus_Leader && !(Global_Rece_decide == 1)){
			AfterRound();
			continue;
		}
		
		if(id==Global_Consensus_Leader && DECIDE_flag){
			u8 tmp_flag = 0;
			while(Global_ACK_TO_Send_Number){
				Global_Go_ACK();
				delay_ms(50);
...
//			Total_time1 = TIM2->CNT + TIM2_Exceed_Times * 9000;
		...
		if(id == Global_Consensus_Leader){
			Global_Leader_Send(GLOBAL_DECIDE,Global_PreCommit_id,Global_PreCommit_Sig);			
		}
		if(id != Global_Consensus_Leader){ 
...
void Global_Start_TIMER(){
	...
	TIM_Prepare_Flag = 0xFF;
	while(TIM_Prepare_Flag){
		if(Tendermint_status == GLOBAL_PREPREPARE){
		...
		}else if(Tendermint_status == GLOBAL_PREVOTE){
...
//		
//		}
		else if(Tendermint_status == GLOBAL_DECIDE){
			
			if((id != Global_Consensus_Leader) && (Global_Rece_next==1))break;
			if(id == Global_Consensus_Leader && Global_Viewchange_Number >= GROUPS_NUMBER)	break;
		
		}else if(Tendermint_status == GLOBAL_NEXT_ROUND){
...
void Global_Leader_Send(u8 PHASE, u8* ID_list, u8 Sig_list[20][64]){
	...
	//发送2f个签名,分为（2f）/3次
	u8 send_number = (2*f+2)/3;
	if(PHASE == GLOBAL_NEXT_ROUND)send_number = ((GROUPS_NUMBER-1)+2)/3;
	for(u8 Packet_id=0;Packet_id<send_number;Packet_id++){
		Send_Data_buff[0] = id;
		Send_Data_buff[1] = PHASE;
		...
		if(Packet_id != send_number-1 )Send_Data_buff[3] = 3;
			...
			if(PHASE == GLOBAL_NEXT_ROUND)Send_Data_buff[3] = (GROUPS_NUMBER-1)-Packet_id*3;
			else Send_Data_buff[3] = (2*f)-Packet_id*3;
		}
