...
/**************************PRECOMMIT**************************/
	...
	while(1){
		if(id != Global_Consensus_Leader){
			while(1){
				if((Global_Rece_precommit == 1) || (Global_Rece_decide == 1) || (Global_Rece_next == 1))break;
				delay_ms(200);
			}
		}
...
/**************************DECIDE**************************/
	...
	while(1){
		...
		if(id != Global_Consensus_Leader){ 
			Global_Follower_Send(GLOBAL_VIEW_CHANGE,my_decide);
		}
...
/**************************Next_Round**************************/
	Tendermint_status = GLOBAL_NEXT_ROUND;
...
void Start_TIMER(){
	...
	while(TIM_Prepare_Flag){
		if(Tendermint_status == PREPREPARE){
			...
			if(id == Group_Leader_ID && ACK_Proposal_Number == N)break;
			else if(id != Group_Leader_ID && Rece_Proposal_flag)break;
			
		}else if(Tendermint_status == PREVOTE){
			
			if(id == Group_Leader_ID && Prepare_Number >= (2 * f + 1)) break;
