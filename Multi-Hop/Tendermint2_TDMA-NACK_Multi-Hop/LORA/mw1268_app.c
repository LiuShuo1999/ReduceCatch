...
//									Total_time = TIM2->CNT + TIM2_Exceed_Times * 9000;
					...
					u8 temp_value = uart3RxBuffer[3];
					if((id == Group_Leader_ID) && (Viewchange_Number <= N-1) && (Rece_Viewchange[temp_id]==0xFF) && (temp_value==1)){
...
//						}
						...
						if(catching_type == PREVOTE && uart3RxBuffer[my_id]==0xFF){
							Need_PREPARE = 1;
...
//						if(verify_sig) Rece_decide = 1;
				...
				else if(verify_flag && (temp_type == GLOBAL_NACK)){
					if(id==Global_Consensus_Leader && temp_id != Global_Consensus_Leader){
						u8 catching_type = uart3RxBuffer[3];
...
//						}
						...
						if(catching_type == GLOBAL_PREVOTE && uart3RxBuffer[my_id]==0xFF){
