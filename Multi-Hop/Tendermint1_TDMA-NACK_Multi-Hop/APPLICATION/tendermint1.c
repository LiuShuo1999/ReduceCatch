...
void Global_NACK_PREPREPARE();
void Global_NACK_PREPARE();
void Global_NACK_COMMIT();
...
/*****************************Local_Consensus**************************************************************************************************************************************************************************/
	...
	while(1){
		...
		if(id!=Group_Leader_ID && Rece_Proposal_flag) break;
...
//	my_choice = 1;
	...
	while(1){
		...
		if(Choice_Truth_Set_Number >= 2*f+1 &&!PREPARE_flag)break;
...
//	my_commit = 1;
	...
	while(1){
		...
		if(Commit_Truth_Set_Number >= 2*f+1 && !COMMIT_flag)break;
...
//		if(Commit_Truth_Set_Number >= 2*f+1 && !COMMIT_flag)break;
	...
	while(1){
		...
		if(Group_Number == 2) delay_ms(500);
		
		if(id==Group_Leader_ID && NEXT_ROUND_flag && Viewchange_Number == Need_to_Wait){

			NEXT_ROUND_flag = 0;
			Leader_Send(NEXT_ROUND,Viewchange_id,Viewchange_Sig);
			break;
		}else if(id != Group_Leader_ID && NEXT_ROUND_flag){
			Send_VIEW_CHANGE();
			NEXT_ROUND_flag=0;
		}else{
			NACK_phase();
...
/*****************************Global_Consensus**************************************************************************************************************************************************************************/
	...
	while(1){
		...
		if(id!=Global_Consensus_Leader && Rece_Proposal_2_flag) break;
...
//	run_slow();
	...
	while(1){
		...
		if(Global_Choice_Truth_Set_Number >= 2*Global_f+1 &&!PREPARE_flag)break;
...
//	my_commit = 1;
	...
	while(1){
		...
		if(Global_Commit_Truth_Set_Number >= 2*Global_f+1 && !COMMIT_flag)break;
...
//		if(Commit_Truth_Set_Number >= 2*f+1 && !COMMIT_flag)break;
	...
	while(1){
		...
		if(id==Global_Consensus_Leader && NEXT_ROUND_flag && Viewchange_Number == Need_to_Wait){
			if(Total_time==0)
		...
		}else if(id != Global_Consensus_Leader && NEXT_ROUND_flag){
...
void NACK_phase(){
	...
	if(id == Group_Leader_ID && Commit_Truth_Set_Number >= 2*f+1){
		...
		else if(Viewchange_Number >= Need_to_Wait){
