...
//        btim_timx_enable(DISABLE); //关闭TIM7
	...
	if (htim == (&tim9_handler))
    {
        ...
        if(!start_flag2){
			...
			if(NTX_proposal_flag){
				if(TIM9_Exceed_Times == 2 * NTX_proposal){
					NTX_proposal_flag = 0;
//					TIM9_Exceed_Times = 0;
				}
				return;
			...
			if(Global_NTX_proposal_flag){
				if(TIM9_Exceed_Times == 2 * Global_NTX_proposal){
					Global_NTX_proposal_flag = 0;
					TIM9_Exceed_Times = 0;
				}
				return;
			}
