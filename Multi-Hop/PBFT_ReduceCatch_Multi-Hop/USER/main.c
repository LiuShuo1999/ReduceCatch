...
//任务优先级
#define PBFT_TASK_PRIO		2
//任务堆栈大小	
#define PBFT_STK_SIZE 		50  
//任务句柄
TaskHandle_t PBFT_Task_Handler;
//任务函数
void PBFT(void *pvParameters);
...
void start_task(void *pvParameters)
{
    ...
    //创建LED0任务
    xTaskCreate((TaskFunction_t )PBFT,     	
                (const char*    )"PBFT",   	
                (uint16_t       )PBFT_STK_SIZE, 
                (void*          )NULL,				
                (UBaseType_t    )PBFT_TASK_PRIO,	
                (TaskHandle_t*  )&PBFT_Task_Handler);   
//    //创建LED1任务
...
//            }
			...
			u8 string_idx=0;
			u8 key_idx = 0;
			for(string_idx = 0; string_idx < sizeof(public_key[3]);){
				public[key_idx] = Sixteen2Ten(public_key[3][string_idx])*16+Sixteen2Ten(public_key[3][string_idx + 1]);
				key_idx = key_idx + 1;
				string_idx = string_idx + 3;
			}
//			public[key_idx] = 0x00;
			key_idx = 0;
			for(string_idx = 0; string_idx < sizeof(private_key[3]);){
				private[key_idx] = Sixteen2Ten(private_key[3][string_idx])*16 + Sixteen2Ten(private_key[3][string_idx + 1]);
				key_idx = key_idx + 1;
				string_idx = string_idx + 3;
			}
//			private[key_idx] = 0x00;
			
			
			for(u8 i = 0 ; i < sizeof(public) ; i++){
				sprintf((char*)Send_Data_buff,"%02x",public[i]);
				Lora_Device_Sta = LORA_TX_STA;
				LoRa_SendData(Send_Data_buff);
				//				delay_ms(200);
			}
			
			for(u8 i = 0 ; i < sizeof(private) ; i++){
				sprintf((char*)Send_Data_buff,"%02x",private[i]);
				Lora_Device_Sta = LORA_TX_STA;
				LoRa_SendData(Send_Data_buff);
//				delay_ms(200);
			}
...
//	
//	init_public_key();
//	hash[0]=2;hash[1]=2;hash[2]=1;hash[3]=1;

//	for(u8 m=0;m<32;m++){
//		sprintf((char*)Send_Data_buff,"private:%02x",private_key[2][m]);
//		Lora_Device_Sta = LORA_TX_STA;
//		LoRa_SendData(Send_Data_buff);// id 0 "Hello LoRa"
//		delay_ms(200);
//	}
//	for(u8 m=0;m<64;m++){
//		sprintf((char*)Send_Data_buff,"public:%02x",public_key[2][m]);
//		Lora_Device_Sta = LORA_TX_STA;
//		LoRa_SendData(Send_Data_buff);// id 0 "Hello LoRa"
//		delay_ms(200);
//	}
//	
//	if (!uECC_sign(private_key[2], hash, 4, sig, curves[0])) {
//		sprintf((char*)Send_Data_buff,"Sign Fail2");
//		Lora_Device_Sta = LORA_TX_STA;
//		LoRa_SendData(Send_Data_buff);// id 0 "Hello LoRa"
//		while(1);
//	}
//	if (!uECC_verify(public_key[2], hash, 4, sig, curves[0])) {
//		sprintf((char*)Send_Data_buff,"verify Fail!!!!!");
//		Lora_Device_Sta = LORA_TX_STA;
//		LoRa_SendData(Send_Data_buff);// id 0 "Hello LoRa"
//		while(1);
//		LED0(0);
...
//		LED2(0);
//	}
//	sprintf((char*)Send_Data_buff,"Success!!!");
//	Lora_Device_Sta = LORA_TX_STA;
//	LoRa_SendData(Send_Data_buff);// id 0 "Hello LoRa"
//	while(1);
