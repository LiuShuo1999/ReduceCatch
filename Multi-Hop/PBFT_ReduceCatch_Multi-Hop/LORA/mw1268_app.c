...
//					start_flag2 = 1;
					u8 nack_type = uart3RxBuffer[3];
					u8 my_id = 3 + Group_Number;
					if(uart3RxBuffer[my_id] == 0xFF){
						if(nack_type == GLOBAL_CHOICE){
							Global_Need_Choice_Flag = 1;
						}else if(nack_type == GLOBAL_COMMIT){
...
void LoRa_ReceData(u8* data)
{
	...
	if(USART3_RX_STA&0x8000)
	{
		...
		led0sta = ~led0sta;
		for(u16 i = 0 ; i < len ; i++){
			data[i] = uart3RxBuffer[i];
		}
		if(LoRa_CFG.tmode==LORA_TMODE_PT)//透明传输
		{	
			//Show_Str_Mid(10,270,USART6_RX_BUF,16,240);//显示接收到的数据
//			LED1(0);
		}else if(LoRa_CFG.tmode==LORA_TMODE_FP)//定向传输
		{
