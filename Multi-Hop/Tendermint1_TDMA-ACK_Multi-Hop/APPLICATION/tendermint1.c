...
/*****************************Local_Consensus**************************************************************************************************************************************************************************/
	...
	while(1){
		...
		if(id == Group_Leader_ID && ACK_Proposal_Number >= Need_to_Wait)break;
	...
	while(1){
		...
		if(id==Group_Leader_ID + 1) delay_ms(500);
		if(id != Group_Leader_ID && !ACK_viewchange)	Send_VIEW_CHANGE();
...
/*****************************Global_Consensus**************************************************************************************************************************************************************************/
	...
	if(id == Global_Consensus_Leader){
		...
		Global_ACK_Proposal[id_idx] = 0x01;
		Global_New_Proposal();
	}
	...
	while(1){
		...
		if(id == Global_Consensus_Leader && Global_ACK_Proposal_Number >= GROUPS_NUMBER)break;
	...
	while(1){
		...
		if((Global_ACK_Choice_Number >= GROUPS_NUMBER) && (Global_Choice_Truth_Set_Number >= 2*Global_f+1))break;
		
		BeforeRound();
		
		if((Global_ACK_Choice_Number >= GROUPS_NUMBER) && (Global_Choice_Truth_Set_Number >= 2*Global_f+1))break;
		
		if(Global_ACK_To_Send_Flag && prepare_flag){
			Global_Send_ACK();
			Global_ACK_To_Send_Flag = 0;
			AfterRound();
			continue;
		}
	...
	commit_flag = 0;
	while(1){
		if((Global_ACK_Commit_Number >= GROUPS_NUMBER) && (Global_Commit_Truth_Set_Number >= 2*Global_f+1))	break;
		
		BeforeRound();
		
		if((Global_ACK_Commit_Number >= GROUPS_NUMBER) && (Global_Commit_Truth_Set_Number >= 2*Global_f+1))	break;
		
		if(Global_ACK_To_Send_Flag && commit_flag){
			Global_Send_ACK();
			Global_ACK_To_Send_Flag = 0;
			AfterRound();
			continue;
	...
	/**********************VIEW_CHANGE*********************************/
//	if(id==Global_Consensus_Leader){
//		delay_ms((GROUPS_NUMBER-Need_to_Wait)*delta);
//	}
	view_change_flag = 0;
	while(1){
		...
		if(Global_ACK_To_Send_Flag && view_change_flag){
			Global_Send_ACK();
			Global_ACK_To_Send_Flag = 0;
			AfterRound();
			continue;
		...
		if(Group_Number == 2) delay_ms(300);
		if(id != Global_Consensus_Leader && !Global_ACK_viewchange)	Global_Send_VIEW_CHANGE();
	...
	while(id != Global_Consensus_Leader){
		BeforeRound();
...
void Global_Send_ACK(){
	...
	Send_Data_buff[0] = id;
	Send_Data_buff[1] = GLOBAL_ACK;
	Send_Data_buff[2] = 2*GROUPS_NUMBER + 1 + 32;
	Send_Data_buff[3] = Rece_Proposal_2_flag;
	for(u8 m=1;m<=GROUPS_NUMBER;m++)Send_Data_buff[3+m] = Global_Choice[m];
	for(u8 m=1;m<=GROUPS_NUMBER;m++)Send_Data_buff[3+m+GROUPS_NUMBER] = Global_Commit[m];
