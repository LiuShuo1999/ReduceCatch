...
///////////////////////////////////////////////////////////////////////////////////////////////////
			if(LoRa_CFG.tmode == LORA_TMODE_PT){
				...
				if(temp_type == PROPOSAL || temp_type == GLOBAL_PROPOSAL_1){
				...
				if(verify_flag && (temp_type == PROPOSAL)){
					...
					//回复ACK，
					//判断是否需要回复
					u8 My_id = id - 4 * (Group_Number - 1)+3;
					...
					u8 temp_value = uart3RxBuffer[3];
					u8 temp_id_idx = temp_id - 4 * (Group_Number - 1);
					if(Commit[temp_id_idx]/*old_value*/ == 0xFF){
						if(temp_value == 0x01){
							Commit_Truth_Set_Number = Commit_Truth_Set_Number + 1;
							Commit[temp_id_idx] = 1;
...
//				else if(verify_flag2 && verify_flag && (temp_type == ACK_VC)){
//					u8 my_id = 2+id;
//					if(uart3RxBuffer[my_id]==1){
//						ACK_viewchange = 1;
//					}
//				}
					...
					ACK_My_Proposal = uart3RxBuffer[3];
					ACK_My_Choice = uart3RxBuffer[id_idx + 3];
					ACK_My_Commit = uart3RxBuffer[id_idx + 3 + N];
					u8 temp_id_idx = temp_id - 4 * (Group_Number - 1);
					if(id == Group_Leader_ID){
						if((ACK_My_Proposal == 1) && (ACK_Proposal[temp_id_idx] == 0xFF)){
							ACK_Proposal[temp_id_idx] = 0x01;
							ACK_Proposal_Number = ACK_Proposal_Number + 1;
						}
					}
					if( (ACK_My_Choice == 1) && (ACK_Choice[temp_id_idx] == 0xFF)){
						ACK_Choice[temp_id_idx] = 0x01;
						ACK_Choice_Number = ACK_Choice_Number + 1;
					}
					if((ACK_My_Commit == 1) && (ACK_Commit[temp_id_idx] == 0xFF)){
						ACK_Commit[temp_id_idx] = 0x01;
...
//				}
					...
					if(ACK_type == GLOBAL_PROPOSAL_1){
						...
						ACK_My_Proposal = uart3RxBuffer[3];
						ACK_My_Choice = uart3RxBuffer[id_idx + 3];
						ACK_My_Commit = uart3RxBuffer[id_idx + 3 + GROUPS_NUMBER];
						if(id == Global_Consensus_Leader){
							if((ACK_My_Proposal == 1) && (Global_ACK_Proposal[temp_id_idx] == 0xFF)){
								Global_ACK_Proposal[temp_id_idx] = 0x01;
								Global_ACK_Proposal_Number = Global_ACK_Proposal_Number + 1;
							}
						}
						if( (ACK_My_Choice == 1) && (Global_ACK_Choice[temp_id_idx] == 0xFF)){
							Global_ACK_Choice[temp_id_idx] = 0x01;
							Global_ACK_Choice_Number = Global_ACK_Choice_Number + 1;
						}
						if((ACK_My_Commit == 1) && (Global_ACK_Commit[temp_id_idx] == 0xFF)){
							Global_ACK_Commit[temp_id_idx] = 0x01;
