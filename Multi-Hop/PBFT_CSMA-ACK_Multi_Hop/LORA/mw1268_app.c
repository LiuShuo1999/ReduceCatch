...
///////////////////////////////////////////////////////////////////////////////////////////////////
			if(LoRa_CFG.tmode == LORA_TMODE_PT){
				...
				u8 verify_flag2 = 1;//proposal hash
				...
				if(temp_id == id) verify_flag=0;
								
				if(verify_flag && (temp_type == PROPOSAL) && temp_id == Group_Leader_ID){
					//Preprepare   Proposal
					...
					if(Rece_Proposal_Number == Proposal_Packet_Number && id != Group_Leader_ID){
					...
					if(ACK_type == PROPOSAL && id == Group_Leader_ID){
						if(ACK_Proposal[temp_id_idx]/*old_value*/ == 0xFF){
							ACK_Proposal_Number = ACK_Proposal_Number + 1;
							ACK_Proposal[temp_id_idx] = 1;
						}
					}
					else if(ACK_type == CHOICE){
						u8 temp2_id = uart3RxBuffer[4];
//						u8 temp2_id_idx = temp2_id - 4 * (Group_Number - 1);
						if(temp2_id == id){
							if(ACK_Choice[temp_id_idx]/*old_value1*/ == 0xFF){
								ACK_Choice_Number = ACK_Choice_Number + 1;
								ACK_Choice[temp_id_idx] = 1;
							}
						}
					}
						...
						if(temp2_id == id){
							if(ACK_Commit[temp_id_idx]/*old_value2*/ == 0xFF){
								ACK_Commit_Number = ACK_Commit_Number + 1;
								ACK_Commit[temp_id_idx] = 1;								
							}
						}						
					}
				}
				//Global_phase
					...
					if(uart3RxBuffer[My_id] == 0xFF){
						...
						Global_ACK_TO_Send[Global_ACK_TO_Send_Number][0] = id;
						Global_ACK_TO_Send[Global_ACK_TO_Send_Number][1] = GLOBAL_ACK;
						Global_ACK_TO_Send[Global_ACK_TO_Send_Number][2] = GLOBAL_CHOICE;
						Global_ACK_TO_Send[Global_ACK_TO_Send_Number][3] = temp_id;
						Global_ACK_TO_Send[Global_ACK_TO_Send_Number][4] = 0x00;
						Global_ACK_TO_Send_Number = Global_ACK_TO_Send_Number + 1;
					}
					...
					else if(ACK_type == GLOBAL_CHOICE){
						u8 temp2_id = uart3RxBuffer[4];
						if(temp2_id == id){
							if(Global_ACK_Choice[temp_id_idx]/*old_value1*/ == 0xFF){
								Global_ACK_Choice_Number = Global_ACK_Choice_Number + 1;
								Global_ACK_Choice[temp_id_idx] = 1;
							}
						}
					}
					else if(ACK_type == GLOBAL_COMMIT){
						u8 temp2_id = uart3RxBuffer[4];
						if(temp2_id == id){
							if(Global_ACK_Commit[temp_id_idx]/*old_value2*/ == 0xFF){
								Global_ACK_Commit_Number = Global_ACK_Commit_Number + 1;
								Global_ACK_Commit[temp_id_idx] = 1;								
							}
						}						
					}
				}
