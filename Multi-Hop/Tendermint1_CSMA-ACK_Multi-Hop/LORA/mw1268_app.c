...
//									Total_time = TIM2->CNT + TIM2_Exceed_Times * 9000;
				...
				else if(verify_flag && (temp_type == ACK)){
					//ACK
					u8 ACK_type;
					...
					if(ACK_type == PROPOSAL){
						...
						if(temp2_id == id){
							if(ACK_Choice[temp_id_idx] == 0xFF){
						...
						u8 temp2_id = uart3RxBuffer[4];
						if(temp2_id == id){
							if(ACK_Commit[temp_id_idx] == 0xFF){
...
//				}
					...
					else if((ACK_type == GLOBAL_VIEW_CHANGE)){
						u8 temp2_id = uart3RxBuffer[4];
						if(temp2_id == id)
