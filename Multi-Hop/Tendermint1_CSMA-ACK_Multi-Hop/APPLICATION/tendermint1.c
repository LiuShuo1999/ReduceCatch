...
//	run_slow();
	...
	while(1){
		...
		if(id != Group_Leader_ID && !ACK_viewchange){
			//vc msg
			Send_VIEW_CHANGE();
		}
		if(id == Group_Leader_ID){
...
//			Send_REPLAY(Total_time1,Total_time,TIM2_Exceed_Times);
		...
		u8 temp_ACK=0;
		Start_TIMER(&temp_ACK,Need_to_Wait);
	}
...
/*****************************Global_Consensus**************************************************************************************************************************************************************************/
	...
	if(id == Global_Consensus_Leader){
		...
		while(1){
			...
			if(Global_ACK_Proposal_Number >= N)break;
...
//	run_slow();
	...
	while(1){
		...
		if(Global_ACK_Commit_Number >= GROUPS_NUMBER) continue;
	...
	while(1){
		...
		if(id != Global_Consensus_Leader && !Global_ACK_viewchange){
			//vc msg
		...
		u8 temp_ACK=0;
		Start_TIMER(&temp_ACK,GROUPS_NUMBER);
	}
