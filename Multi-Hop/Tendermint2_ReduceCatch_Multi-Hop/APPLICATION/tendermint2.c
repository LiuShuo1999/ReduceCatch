...
/*****************************Local_Consensus**************************************************************************************************************************************************************************/
	...
	for(u8 ntx=0;ntx<NTX_proposal;ntx++){
		...
		if(id != Group_Leader_ID && Rece_Proposal_flag==1)break;
...
/**************************PRECOMMIT**************************/
	...
	for(u8 ntx=0;ntx<NTX;ntx++){
		...
		if(id != Group_Leader_ID && Rece_precommit == 1){        
...
/**************************DECIDE**************************/
	...
	for(u8 ntx=0;ntx<NTX;ntx++){
		...
		if(id == Group_Leader_ID && Viewchange_Number >= N)	break;
...
//			Send_REPLAY(Total_time1,Total_time,TIM2_Exceed_Times);
		}else if(id!=Group_Leader_ID && Rece_next==1){
//			Send_REPLAY(Total_time1,Total_time,TIM2_Exceed_Times);
		}else{
			delay_ms(1000);
		}
...
/*****************************Global_Consensus**************************************************************************************************************************************************************************/
	...
	for(u8 ntx=0;ntx<Global_NTX_proposal;ntx++){
		...
		if(id != Global_Consensus_Leader && Rece_Proposal_2_flag==1)break;
...
/**************************PRECOMMIT**************************/
	...
	for(u8 ntx=0;ntx<Global_NTX;ntx++){
		...
		if(id != Global_Consensus_Leader && Global_Rece_precommit == 1){        
...
/**************************DECIDE**************************/
	...
	for(u8 ntx=0;ntx<Global_NTX;ntx++){
		...
		if(id == Global_Consensus_Leader){
			...
			for(u8 m=0;m<Extera_slot;m++)Global_Leader_Send(GLOBAL_DECIDE,Global_PreCommit_id,Global_PreCommit_Sig);
		}
...
//		BeforeRound();
		...
		if((Global_Viewchange_Number >= GROUPS_NUMBER || Global_Rece_next==1) && Total_time == 0){
			Total_time = TIM2->CNT + TIM2_Exceed_Times * 9000;
		}
		...
		else if(id!=Global_Consensus_Leader && Global_Rece_next==1){
			Send_REPLAY(Total_time1,Total_time,TIM2_Exceed_Times);
		}
		else{
			delay_ms(1000);
...
//			}
			test_finish();
			if(Global_Need_decide && (Global_Precommit_Number >= (2*Global_f+1))){
			...
			if(Global_Prepare_Number < (2*Global_f+1)){
				for(u8 m=0;m<phase_NTX;m++){
					Global_Leader_Catching(GLOBAL_PREVOTE,Global_Rece_Prepare);
					delay_ms(Packet_Internal_delay);
				}
...
//				}	
//			}
			else if(Global_Viewchange_Number < GROUPS_NUMBER){
		...
		while(1){
			...
			if(Global_Need_PRECOMMIT && ((Global_Rece_precommit == 1)||(Global_Rece_decide == 1)||(Global_Rece_next==1))){
				Global_Need_PRECOMMIT = 0;
