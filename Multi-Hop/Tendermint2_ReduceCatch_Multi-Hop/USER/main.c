...
//任务优先级
#define TENDERMINT2_TASK_PRIO		2
//任务堆栈大小	
#define TENDERMINT2_STK_SIZE 		512  
//任务句柄
TaskHandle_t TENDERMINT2_Task_Handler;
//任务函数
void TENDERMINT2(void *pvParameters);
...
void start_task(void *pvParameters)
{
    ...
    //创建LED0任务
    xTaskCreate((TaskFunction_t )TENDERMINT2,     	
                (const char*    )"TENDERMINT2",   	
                (uint16_t       )TENDERMINT2_STK_SIZE, 
                (void*          )NULL,				
                (UBaseType_t    )TENDERMINT2_TASK_PRIO,	
                (TaskHandle_t*  )&TENDERMINT2_Task_Handler);   
//    //创建LED1任务
