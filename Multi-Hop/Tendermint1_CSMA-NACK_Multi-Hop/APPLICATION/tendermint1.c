...
//	run_slow();
	...
	while(1){
		...
		//ACK
		NACK_phase();
		if(id == Group_Leader_ID && NEXT_flag){
			NEXT_flag = 0;
...
//	run_slow();
	...
	while(1){
		...
		if(Global_Commit_Truth_Set_Number >= 2*Global_f+1){
			break;
		}
	...
	while(1){
		...
		//ACK
		Global_NACK_phase();
		if(id == Global_Consensus_Leader && NEXT_flag){
			NEXT_flag = 0;
			if(Total_time==0)
...
void Global_NACK_phase(){
	...
	if(!Rece_Proposal_2_flag && id != Global_Consensus_Leader){
