...
///////////////////////////////////////////////////////////////////////////////////////////////////
			if(LoRa_CFG.tmode == LORA_TMODE_PT){
				...
				else if(uart3RxBuffer[1]==GLOBAL_CATCHING && uart3RxBuffer[3]==GLOBAL_PROPOSAL_2){
					verify_flag2 = 0;
					temp_datasize = uart3RxBuffer[2];
					prefix_proposal = 3;
				}
				...
				if(verify_flag && !verify_flag2 && uart3RxBuffer[1]==CATCHING && uart3RxBuffer[3]==PROPOSAL){
					...
					if(Rece_Proposal_Number == Proposal_Packet_Number && !Rece_Proposal_flag){
						Rece_Proposal_flag = 1;
					}
					
				}
				else if(verify_flag && (temp_type == CHOICE)){
					u8 temp_choice = uart3RxBuffer[3];
					u8 temp_id_idx = temp_id - 4 * (Group_Number - 1);
					if(Choice[temp_id_idx] == 0xFF){
						Choice[temp_id_idx] = temp_choice;
						if(temp_choice)
							Choice_Truth_Set_Number++;
						else 
							Choice_False_Set_Number++;
					}
				 }else if(verify_flag && (temp_type == COMMIT)){
					u8 temp_commit = uart3RxBuffer[3];	
					u8 temp_id_idx = temp_id - 4 * (Group_Number - 1);
					if(Commit[temp_id_idx] == 0xFF){
						Commit[temp_id_idx] = temp_commit;
						if(temp_commit)
							Commit_Truth_Set_Number++;
						else 
							Commit_False_Set_Number++;
					}
...
//									Total_time = TIM2->CNT + TIM2_Exceed_Times * 9000;
					...
					u8 temp_value = uart3RxBuffer[3];
					if((id == Group_Leader_ID) && (Viewchange_Number <= Need_to_Wait) && (Rece_Viewchange[temp_id]==0xFF) && (temp_value==1)){
					...
					if(id == Group_Leader_ID && temp_id != Group_Leader_ID){
						if(catching_type == NEXT_ROUND){
							Need_next = 1;
						}
					}else if(temp_id == Group_Leader_ID){
						if(catching_type == DECIDE && uart3RxBuffer[my_id]==0xFF){
							Need_VIEWCHANGE = 1;
						}
					}		
				 }
...
//					}
//				}
				else if(verify_flag && !verify_flag2 && uart3RxBuffer[1]==GLOBAL_CATCHING && uart3RxBuffer[3]==GLOBAL_PROPOSAL_2 && id == Global_Consensus_Leader){
					Global_Need_Proposal_Flag = 1;
				}
				else if(verify_flag && (temp_type == GLOBAL_PROPOSAL_1) && temp_id != Group_Leader_ID){
				...
				else if(verify_flag && (temp_type == GLOBAL_SIG) && temp_id != Group_Leader_ID){
					if(Global_Rece_Proposal_Number[tmp_group_number] == Global_Proposal_Packet_Number[tmp_group_number]){
						//验证2f个签名
						u8 Tmp_id[20];
						u8 Tmp_Sig[20][64];
						...
						if(Global_Rece_Sig_Number[tmp_group_number]>=Need_to_Wait-1){
				...
				else if(verify_flag2 && verify_flag && (temp_type == GLOBAL_CHOICE)){
					//Prepare	Choice
					u8 temp_value = uart3RxBuffer[3];
					
					u8 temp_id_idx = tmp_group_number;
					if(Global_Choice[temp_id_idx] == 0xFF){
						if(temp_value == 0x01){
							Global_Choice_Truth_Set_Number = Global_Choice_Truth_Set_Number + 1;
							Global_Choice[temp_id_idx] = 1;
						}
					}
					...
					u8 temp_value = uart3RxBuffer[3];
					u8 temp_id_idx = tmp_group_number;
					if(Global_Commit[temp_id_idx]/*old_value*/ == 0xFF){
						if(temp_value == 0x01){
							Global_Commit_Truth_Set_Number = Global_Commit_Truth_Set_Number + 1;
							Global_Commit[temp_id_idx] = 1;
						}
					}
				}
					...
					if(uart3RxBuffer[my_id] == 0xFF){
						if(catching_type == GLOBAL_CHOICE){
							Global_Need_Choice_Flag = 1;
						}else if(catching_type == GLOBAL_COMMIT){
							Global_Need_Commit_Flag = 1;
						}
					}
					if(id == Global_Consensus_Leader && temp_id != Global_Consensus_Leader){
						if(catching_type == GLOBAL_NEXT_ROUND){
							Global_Need_next = 1;
						}
					}else if(temp_id == Global_Consensus_Leader){
						if(catching_type == GLOBAL_DECIDE && uart3RxBuffer[my_id]==0xFF){
