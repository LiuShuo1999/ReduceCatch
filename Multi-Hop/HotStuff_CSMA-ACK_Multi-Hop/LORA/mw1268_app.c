...
///////////////////////////////////////////////////////////////////////////////////////////////////
			if(LoRa_CFG.tmode == LORA_TMODE_PT){
				...
				if(verify_flag && (temp_type == PROPOSAL)){
					...
					u8 old_value;
					if((id == Group_Leader_ID) && (Prepare_Number <= N-1) && (Rece_Prepare[temp_id]==0xFF) && (temp_value==1)){
...
//									Total_time = TIM2->CNT + TIM2_Exceed_Times * 9000;
					...
					if((ACK_type == PREPARE) && (temp2_id == id)){
						ACK_prepare = 1;
					}
					else if((ACK_type == PRECOMMIT) && (temp2_id == id)){
						ACK_precommit = 1;
					}
					else if((ACK_type == COMMIT) && (temp2_id == id)){
						ACK_commit = 1;
					}
...
//						if(verify_sig) Rece_decide = 1;
					...
					else if((ACK_type == GLOBAL_PREPARE) && (temp2_id == id)){
